// Generated by native/Scripts/generate-tables.sh from Python's html.entities.html5
// (WHATWG HTML5 named character references). Do not edit by hand.

/// HTML5 named character references (CommonMark §2.5), decoded lazily from a compact blob.
/// Each record is `name<TAB>hex-scalars` so that `&Tab;` and `&NewLine;` stay ASCII-safe.
enum EntityTable {
  static let map: [Substring: String] = {
    var m: [Substring: String] = [:]
    m.reserveCapacity(2125)
    for line in blob.split(separator: "\n", omittingEmptySubsequences: true) {
      guard let tab = line.firstIndex(of: "\t") else { continue }
      var value = String.UnicodeScalarView()
      for hex in line[line.index(after: tab)...].split(separator: " ") {
        if let v = UInt32(hex, radix: 16), let s = Unicode.Scalar(v) { value.append(s) }
      }
      m[line[..<tab]] = String(value)
    }
    return m
  }()
