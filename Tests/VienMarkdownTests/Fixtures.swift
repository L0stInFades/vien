...
/// Shared access to the fixtures under Tests/Fixtures (spec JSON and the lossless corpus).
enum Fixtures {
  static let dir = URL(fileURLWithPath: #filePath).deletingLastPathComponent().deletingLastPathComponent().appendingPathComponent("Fixtures")
  ...
  static func examples(_ path: String) -> [Example] {
    guard let data = try? Data(contentsOf: dir.appendingPathComponent(path)) else { return [] }
    return (try? JSONDecoder().decode([Example].self, from: data)) ?? []
  }

  static let commonMark = examples("specs/commonmark.0.31.2.json")
  static let gfm = examples("specs/gfm.0.29.json")

  static var corpus: [URL] {
    let dir = dir.appendingPathComponent("corpus/lossless")
